...
function compressedDataToJson(compressedData: string): Draft {
  ...
  const actions = rawActions
    .split(',')
    .map((a) => a.split(''))
    .map(([scope, object, action, visibility, player]) => ({
...
function validateCivs(civList: number[]): void {
  civList.map((civId) => {
    if (!civIds.has(civId)) throw new InvalidCivError(civId);
  });
}

function validateMaps(mapList: number[]): void {
  mapList.map((mapId) => {
    if (!mapIds.has(mapId)) throw new InvalidMapError(mapId);
  });
}

function validateAction(action: Action): void {
  if (!actionTypeSet.has(action.type)) throw new InvalidActionError(action);
  if (!actionScopeSet.has(action.scope)) throw new InvalidActionError(action);
  if (!actionVisibilitySet.has(action.visibility)) throw new InvalidActionError(action);
  if (!(action.captain in Captains)) throw new InvalidActionError(action);
  if (!actionObjectSet.has(action.object)) throw new InvalidActionError(action);
}
...
export function inflatePresets(presets: string[]): Draft[] {
